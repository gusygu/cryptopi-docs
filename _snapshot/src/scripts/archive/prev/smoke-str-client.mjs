// str client smoke (discovering page + API)
...
async function findPage(){
  const pages = [OVERRIDE_PAGE, "/str-aux","/","/home"].filter(Boolean);
  for (const p of pages){ const r = await getHTML(p); if (r.ok && (r.ctype||"").includes("text/html")) return {p,r}; }
  return null;
}
async function findAPI(){
  const apis = [OVERRIDE_API, "/api/str-aux/latest","/api/str-aux/matrix"].filter(Boolean);
...
(async function main(){
  try{
    ...
    assert(page, "no STR page available");
    ok(`PAGE ${page.p}`);
    const api = await findAPI();
    assert(api, "no STR API available");
    ok(`API ${api.a}`);
    ok("STR client smoke ✓");
    process.exit(0);
  }catch(e){ fail("STR client smoke", e); }
})();
