// cin client smoke (discovering page + API)
...
async function findPage(){
  const pages = [OVERRIDE_PAGE, "/dynamics","/","/home"].filter(Boolean);
  for (const p of pages){ const r = await getHTML(p); if (r.ok && (r.ctype||"").includes("text/html")) return {p,r}; }
  return null;
}
async function findAPI(){
  const apis = [OVERRIDE_API, "/api/cin-aux","/api/matrices/latest"].filter(Boolean);
...
(async function main(){
  try{
    ...
    if (!page) warn("no CIN page found (continuing)"); else ok(`PAGE ${page.p}`);
    const api = await findAPI();
    assert(api, "no CIN API available");
    assert(api.r.json!=null, "CIN payload invalid");
    ok(`API ${api.a}`);
    ok("CIN client smoke ✓");
    process.exit(0);
  }catch(e){ fail("CIN client smoke", e); }
})();
